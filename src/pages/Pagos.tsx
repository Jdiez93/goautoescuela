...
import { Car, ArrowLeft, CreditCard, BookOpen, CheckCircle, Loader2, AlertCircle } from "lucide-react";
...
export default function Pagos() {
  ...
  // Fetch payments to compute remaining classes
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      <main className="container mx-auto px-4 py-10 max-w-5xl">
        ...
        {success === "true" && (
          <div className="mb-6 p-4 rounded-xl bg-accent border border-primary/20 flex items-center gap-3">
            <CheckCircle className="w-5 h-5 text-primary shrink-0" />
            <p className="text-sm text-accent-foreground font-medium">
              ¡Pago realizado con éxito! Las clases se añadirán a tu saldo en breve.
            </p>
          </div>
        )}
        {canceled === "true" && (
        ...
        <Card className="mb-10">
          <CardHeader>
            <CardTitle className="text-xl flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-primary" /> Mi saldo de clases
            </CardTitle>
          </CardHeader>
          <CardContent>
            {paymentsLoading ? (
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                <div className="text-center sm:text-left">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Clases disponibles</p>
                </div>
                ...
                <div className="text-center sm:text-left">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Total compradas</p>
                </div>
              </div>
            )}
          </CardContent>
        ...
        <div className="grid sm:grid-cols-3 gap-6 mb-12">
          {PACKS.map((pack) => (
            <Card
              key={pack.key}
              className={`relative flex flex-col transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--card-shadow-hover)] ${
                pack.popular ? "border-primary ring-2 ring-primary/20" : ""
              }`}
            >
              {pack.popular && (
                <Badge className="absolute -top-3 left-1/2 -translate-x-1/2 bg-hero-gradient text-primary-foreground border-0">
                  Más popular
                </Badge>
              )}
              ...
              <CardContent className="text-center flex-1">
                <p className="text-4xl font-bold font-['Space_Grotesk']">
                  {pack.price}€
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  {(pack.price / pack.classes).toFixed(2)}€ / clase
                </p>
              </CardContent>
              <CardFooter>
                <Button
